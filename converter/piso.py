...
Supports both INT8 (256 levels) and INT4 (16 levels) quantization grids.
"""
...
# ---------------------------------------------------------------------------
# Shared PyTorch helper (parameterized on quantization grid)
# ---------------------------------------------------------------------------


def _compute_piso_scales_pytorch(
    W: torch.Tensor,
    h_diag: torch.Tensor,
    num_coarse: int,
    num_fine: int,
    ...
    """PyTorch implementation of symmetric PiSO grid search.

    Parameterized on the quantization grid so the same code serves both
    INT8 (qmin=-128, qmax=127, divisor=127.0) and symmetric INT4
    (qmin=-8, qmax=7, divisor=7.0).

    Args:
        W: [out_features, in_features] weight matrix.
        h_diag: [in_features] Hessian diagonal.
        num_coarse: number of coarse candidates (0.5x–2.0x of reference).
        num_fine: number of fine candidates (0.95x–1.05x of coarse winner).
        qmin: minimum quantized integer value.
        qmax: maximum quantized integer value.
        divisor: scale divisor (max positive representable value).

    Returns:
        [out_features, 1] float32 optimal per-row scales.
    """
    ...
    for start in range(0, out_features, batch_size):
        ...
        s_absmax = W_batch.abs().amax(dim=1, keepdim=True) / divisor  # [B, 1]
        ...
        q = _round_half_away_from_zero(W_3d / s_3d).clamp(qmin, qmax)
        ...
        q_fine = _round_half_away_from_zero(W_3d / s_3d_fine).clamp(qmin, qmax)
...
def _compute_piso_scales_asymmetric_pytorch(
    W: torch.Tensor,
    h_diag: torch.Tensor,
    num_coarse: int,
    num_fine: int,
    ...
    """PyTorch implementation of asymmetric PiSO grid search.

    Parameterized on the quantization grid so the same code serves both
    INT8 (qmin=-128, qmax=127, coarse [0.8, 1.2]) and INT4
    (qmin=-8, qmax=7, coarse [0.5, 2.0]).

    For asymmetric quantization, the zero-point is
    ``zp = clamp(round(qmin - w_min / s), qmin, qmax)``.

    Args:
        W: [out_features, in_features] weight matrix.
        h_diag: [in_features] Hessian diagonal.
        num_coarse: number of coarse candidates.
        num_fine: number of fine candidates.
        qmin: minimum quantized integer value.
        qmax: maximum quantized integer value.
        coarse_low: coarse search lower factor (e.g. 0.8 or 0.5).
        coarse_high: coarse search upper factor (e.g. 1.2 or 2.0).

    Returns:
        (scales, zero_points):
            scales: [out_features, 1] float32 optimal per-row scales.
            zero_points: [out_features, 1] int8 per-row zero-points.
    """
    out_features, in_features = W.shape

    scales = torch.empty(out_features, 1, dtype=torch.float32, device=W.device)
    zero_points = torch.empty(out_features, 1, dtype=torch.int8, device=W.device)

    coarse_factors = torch.linspace(coarse_low, coarse_high, num_coarse, device=W.device, dtype=torch.float32)
    fine_factors = torch.linspace(0.95, 1.05, num_fine, device=W.device, dtype=torch.float32)
    ...
    divisor = float(qmax - qmin)  # INT8: 255, INT4: 15
    ...
    for start in range(0, out_features, batch_size):
        ...
        s_ref = (range_val / divisor).clamp(min=1e-10)  # [B, 1]
        ...
        W_3d = W_batch.unsqueeze(1)  # [B, 1, D]
        s_3d = s_coarse.unsqueeze(2)  # [B, C, 1]
        zp_3d = _round_half_away_from_zero(qmin - w_min.unsqueeze(2) / s_3d).clamp(qmin, qmax)  # [B, C, 1]
        q = _round_half_away_from_zero(W_3d / s_3d + zp_3d).clamp(qmin, qmax)
        ...
        s_3d_fine = s_fine.unsqueeze(2)
        zp_3d_fine = _round_half_away_from_zero(qmin - w_min.unsqueeze(2) / s_3d_fine).clamp(qmin, qmax)
        q_fine = _round_half_away_from_zero(W_3d / s_3d_fine + zp_3d_fine).clamp(qmin, qmax)
        dequant_fine = (q_fine - zp_3d_fine) * s_3d_fine
        cost_fine = ((W_3d - dequant_fine) ** 2 * h_diag).sum(dim=2)
        best_fine_idx = cost_fine.argmin(dim=1)

        scales[start:end, 0] = s_fine[torch.arange(B), best_fine_idx]
        zero_points[start:end, 0] = zp_3d_fine.squeeze(2)[torch.arange(B), best_fine_idx].to(torch.int8)

    return scales, zero_points


# ---------------------------------------------------------------------------
# Public INT8 entry points (thin wrappers around shared helpers)
# ---------------------------------------------------------------------------
...
def compute_piso_scales_int8(
    ...
    if _HAS_TRITON and torch.cuda.is_available():
        try:
            ...
            if result is not None:
                logger.debug("PiSO: Triton symmetric INT8, %dx%d", out_features, in_features)
    ...
    # ── PyTorch fallback ──
    return _compute_piso_scales_pytorch(W, h_diag, num_coarse, num_fine, -128, 127, 127.0)
...
def compute_piso_scales_int8_asymmetric(
    ...
    if _HAS_TRITON and torch.cuda.is_available():
        try:
            ...
            if result is not None:
                ...
                logger.debug("PiSO: Triton asymmetric INT8, %dx%d", out_features, in_features)
    ...
    # ── PyTorch fallback ──
    return _compute_piso_scales_asymmetric_pytorch(
        W, h_diag, num_coarse, num_fine,
        qmin=-128, qmax=127, coarse_low=0.8, coarse_high=1.2,
    )


# ---------------------------------------------------------------------------
# Public INT4 entry points
# ---------------------------------------------------------------------------


def compute_piso_scales_int4(
    W: torch.Tensor,
    hessian_diag: torch.Tensor,
    num_coarse: int = 128,
    num_fine: int = 16,
) -> torch.Tensor:
    """Compute PiSO-optimal per-row scales for symmetric INT4 quantization.

    Like :func:`compute_piso_scales_int8` but for the INT4 grid
    ``{-8, ..., 7}`` (divisor 7.0).
    ...
    if _HAS_TRITON and torch.cuda.is_available():
        try:
            W_gpu = W.float().to("cuda", non_blocking=True)
            h_gpu = h_diag.to("cuda", non_blocking=True)
            from .piso_triton import compute_piso_scales_int4_triton
            result = compute_piso_scales_int4_triton(W_gpu, h_gpu, num_coarse, num_fine)
            if result is not None:
                logger.debug("PiSO: Triton symmetric INT4, %dx%d", out_features, in_features)
                return result.to(W.device)
        except torch.cuda.OutOfMemoryError:
            if torch.cuda.is_available():
                torch.cuda.empty_cache()
            logger.debug("PiSO: OOM on GPU (INT4 symmetric), falling back to CPU")
        except Exception as exc:
            logger.debug("PiSO: Triton INT4 symmetric failed (%s), falling back to CPU", exc)

    # ── PyTorch fallback ──
    return _compute_piso_scales_pytorch(W, h_diag, num_coarse, num_fine, -8, 7, 7.0)


def compute_piso_scales_int4_asymmetric(
    W: torch.Tensor,
    hessian_diag: torch.Tensor,
    num_coarse: int = 128,
    num_fine: int = 16,
) -> tuple[torch.Tensor, torch.Tensor]:
    """Compute PiSO-optimal per-row scales and zero-points for asymmetric INT4.

    The INT4 grid ``{-8, ..., 7}`` has only 16 levels, so the gap between
    absmax and PiSO-optimized scales is wider than INT8.  The coarse search
    range is widened to ``[0.5x, 2.0x]`` of the range-based reference.
    ...
    Args:
        W: [out_features, in_features] weight matrix (float32)
        hessian_diag: [in_features] diagonal of H = X^T X
        num_coarse: number of coarse candidates (0.5x–2.0x range-based)
        num_fine: number of fine candidates (0.95x–1.05x coarse winner)
    ...
    if _HAS_TRITON and torch.cuda.is_available():
        try:
            W_gpu = W.float().to("cuda", non_blocking=True)
            h_gpu = h_diag.to("cuda", non_blocking=True)
            from .piso_triton import compute_piso_scales_int4_asymmetric_triton
            result = compute_piso_scales_int4_asymmetric_triton(W_gpu, h_gpu, num_coarse, num_fine)
            if result is not None:
                scales_gpu, zp_gpu = result
                logger.debug("PiSO: Triton asymmetric INT4, %dx%d", out_features, in_features)
                return scales_gpu.to(W.device), zp_gpu.to(W.device)
        except torch.cuda.OutOfMemoryError:
            if torch.cuda.is_available():
                torch.cuda.empty_cache()
            logger.debug("PiSO: OOM on GPU (INT4 asymmetric), falling back to CPU")
        except Exception as exc:
            logger.debug("PiSO: Triton INT4 asymmetric failed (%s), falling back to CPU", exc)
